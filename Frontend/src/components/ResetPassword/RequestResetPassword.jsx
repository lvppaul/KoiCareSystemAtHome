...
const RequestResetPassword = ({ show, setShow, userEmail }) => {
  ...
  const handleSubmitResetPassword = async (e) => {
    ...
    try {
      ...
      const response = await requestResetPassword(userEmail ? userEmail : email);
      if (response === 200) {
  ...
  return (
    <>
      {!userEmail ? (
        <Link as Button onClick={() => setShow(true)} className="forgot-password">
          <p>Forgot password?</p>{" "}
        </Link>
      ) : null}
