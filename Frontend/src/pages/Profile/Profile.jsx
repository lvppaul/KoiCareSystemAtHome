...
import { Tabs, Tab, Button, Container, Row, Col, Image, ListGroup } from 'react-bootstrap';
...
const Profile = () => {
  ...
  const handleAvatarChange = async (e) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ...
      try {   
        await updateAccount({ ...details, avatar: storageRef.fullPath });
          ...
          <Container className='p-2' style={{ borderRadius: '0 10px 10px 10px' , backgroundColor: '#FFFFFF' }}>
            <Row className="mb-3">
              <Col md={4} className="d-flex flex-column align-items-center">
                <Image className='mt-5' style={{ maxBlockSize: '250px' }} src={avatar} alt='avatar' roundedCircle fluid />
                <input
                  type="file"
                  id="avatarInput"
                  style={{ display: 'none' }}
